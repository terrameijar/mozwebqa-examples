...
def test_login(base_url, mozwebqa, variables):
    """Users can log in"""
    login_page = LoginPage(base_url, mozwebqa.selenium)
    login_page.open()
    messages_page = login_page.login(variables['username'], variables['password'])
    assert messages_page.notification == 'You were logged in'


def test_invalid_username(base_url, mozwebqa, variables):
...
def test_invalid_password(base_url, mozwebqa, variables):
...
def test_logout(base_url, mozwebqa, variables):
    """Make sure logout works"""
    messages_page = MessagesPage(base_url, mozwebqa.selenium)
    messages_page.open()
